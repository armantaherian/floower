...
from .const import (
    ...
    CMD_WRITE_RGB_COLOR,
    CMD_WRITE_STATE,
    WIND_CYCLE_SECONDS_DEFAULT,
    WIND_MAX_OPEN_DEFAULT,
    WIND_MIN_OPEN_DEFAULT,
    WIND_TICK_SECONDS_DEFAULT,
    WIND_TRANSITION_MS_DEFAULT,
)
...
class FlowerLightDevice:
    ...
    def __init__(self, ble_device, name: str | None = None) -> None:
        ...
        self._firmware = None
        self._serial = None
        self._wind_task: asyncio.Task | None = None
        self._wind_started_at = 0.0
    ...
    @property
    def wind_mode_active(self) -> bool:
        """Return whether wind mode task is running."""
        return self._wind_task is not None and not self._wind_task.done()

    async def start_wind_mode(
        self,
        min_open: int = WIND_MIN_OPEN_DEFAULT,
        max_open: int = WIND_MAX_OPEN_DEFAULT,
        cycle_seconds: float = WIND_CYCLE_SECONDS_DEFAULT,
        tick_seconds: float = WIND_TICK_SECONDS_DEFAULT,
        transition_ms: int = WIND_TRANSITION_MS_DEFAULT,
    ) -> None:
        """Start petal wind mode animation."""
        if not self.is_connected:
            raise BleakError("Device not connected")

        min_open = max(0, min(100, int(min_open)))
        max_open = max(0, min(100, int(max_open)))
        if min_open >= max_open:
            raise ValueError("Wind min_open must be lower than max_open")
        cycle_seconds = max(1.0, float(cycle_seconds))
        tick_seconds = max(0.2, float(tick_seconds))
        transition_ms = max(0, int(transition_ms))

        await self.stop_wind_mode()
        self._wind_started_at = time.monotonic()
        self._wind_task = asyncio.create_task(
            self._run_wind_mode(
                min_open=min_open,
                max_open=max_open,
        ...
        self._is_on = True
        _LOGGER.debug(
            "Started wind mode: min=%s max=%s cycle=%.2fs tick=%.2fs transition=%sms",
            min_open,
            max_open,
            cycle_seconds,
            tick_seconds,
    ...
    async def stop_wind_mode(self) -> None:
        """Stop wind mode animation."""
        if not self._wind_task:
            return

        task = self._wind_task
    ...
    def _cancel_wind_mode_task(self) -> None:
        """Cancel wind task without awaiting (for sync callbacks)."""
        if self._wind_task and not self._wind_task.done():
            self._wind_task.cancel()
        self._wind_task = None

    async def _run_wind_mode(
        self,
        min_open: int,
        max_open: int,
        cycle_seconds: float,
        ...
        try:
            while self.is_connected:
                elapsed = time.monotonic() - self._wind_started_at
                phase = elapsed * (2 * math.pi / cycle_seconds)

                base_wave = (math.sin(phase) + 1.0) / 2.0
                gust_wave = (math.sin(phase * 0.47 + 1.2) + 1.0) / 2.0
                jitter = (random.random() - 0.5) * 0.1
                smooth = max(0.0, min(1.0, base_wave * 0.78 + gust_wave * 0.22 + jitter))
                target_level = round(min_open + (max_open - min_open) * smooth)

                self._petal_position = target_level
                await self._send_command(
                    CMD_WRITE_PETALS,
                    {"l": target_level, "t": transition_ms},
                )
            ...
            _LOGGER.debug("Wind mode task cancelled")
            raise
        except Exception as e:
            _LOGGER.warning("Wind mode stopped due to error: %s", e)
        finally:
            self._wind_task = None
